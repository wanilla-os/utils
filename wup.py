...
def create():
    ...
    with open('install_'+pkg_name+".sh", 'w') as script:
        ...
        now = run("pwd")[:-1]
        os.system('echo "# start commands for install" > '+now+"/"+pkg_name+".history")
        ...
        os.system('wget --no-check-certificate '+main_archive+" -O "+pkg_name+'.'+ext)
        script.write('wget --no-check-certificate '+main_archive+" -O "+pkg_name+'.'+ext+"\n")
        ...
        for r in rs:
            ...
            text = spl2[0]
            post = spl2[1]
            sripts = pre + '\n' + 'echo "'+text.replace('"', r'\"') +'" '+r[0]+r[1]+' \n'+post
        script.write(sripts+"\n")
        os.system("echo "+pkg_name+"-"+pkg_ver+" >> $HOME/.cache/wup/installed.txt")
...
def install():
    ...
    print(os.system("touch "+os.environ['HOME']+"/.cache/wup/installed.txt"))
...
def untar(pkg, pname=""):
    ...
    if d in all():
        print('\n\nalready installed\n\n\n')
        return
    run("echo "+d+" >> "+os.environ['HOME']+"/.cache/wup/installed.txt")
...
def all():
    return run("cat "+os.environ['HOME']+"/.cache/wup/installed.txt").split('\n')
...
def main(args):
    if len(args)==1:
        ...
        if args[2] in all():
            print('\n\nalready installed\n\n\n')
            return
        run("echo "+args[2]+" >> "+os.environ['HOME']+"/.cache/wup/installed.txt")
    elif args[1] == "install":
        install()
    elif args[1] == "create":
        create()
